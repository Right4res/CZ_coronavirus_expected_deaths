...
setwd("C:/Users/phili/OneDrive/Git/CZ_coronavirus_expected_deaths") # set to own WD

date_today <- as.Date("22/10/2020", format = "%d/%m/%Y")
date_model <- as.Date("22/10/2020", format = "%d/%m/%Y")
...
# CZ_seasonal_adjustment <- read.csv("CZ_seasonal_mortality.csv") #not use
# 
# CZ_population_by_age <- read.csv("populace_dle_veku.csv", colClasses=c('numeric', 'numeric')) # not used
...
CZ_cases_by_age_projected <- CZ_dates_age_categories_future %>%
  ...
  mutate(cases = if_else(is.na(cases) == TRUE, 0, cases),
         ...
         R_3 = case_when(week == 42 ~ 1.4, # lockdown now
                         ...
                         week == 51 ~ 0.5,
                         week == 52 ~ 0.5)^(1/7),
         R_4 = case_when(week == 42 ~ 1.4, # lockdown  now, but lifted in two weeks
                         week == 43 ~ 1.3,
                         week == 44 ~ 0.9,
                         week == 45 ~ 0.9,
                         week == 46 ~ 1.3,
...
## Expected Deaths based on todays cases

```{r, warning=FALSE}

CZ_expected_deaths_timeline_summary <- CZ_expected_deaths_timeline %>%
  mutate(month = month(date_expected_death)) %>%
  group_by(month) %>%
  summarise(expected_deaths = sum(expected_deaths))
...
CZ_expected_deaths_timeline %>% 
  ...
  theme_light() +
  theme(panel.grid.minor = element_blank()) +
  labs(y = "Deaths (7 day rolling average)", x = "", title = "Expected daily deaths (brown) vs actual deaths (black)", subtitle = "Based solely on cases registered as of today")
```
